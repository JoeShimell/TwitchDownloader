    ...
    public class ChatRenderer
    {
        ...
        public async Task RenderVideoAsync(IProgress<ProgressReport> progress, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                progress.Report(new ProgressReport() { reportType = ReportType.Message, data = "Fetching Youtube Emojis" });
                Dictionary<string, SKBitmap> youtubeEmojiCache = await Task.Run(() => YoutubeHelper.GetYoutubeEmojis(cacheFolder));
...
/*                      List<GifEmote> currentGifEmotes = new List<GifEmote>();
                        ...
                        sectionImage = DrawYTMessage(sectionImage, imageList, renderOptions, messageFont, chatYTJson[i], canvasSize, ref drawPos, ref default_x, emojiCache, youtubeEmojiCache);
